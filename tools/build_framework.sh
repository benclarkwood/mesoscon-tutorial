...
build_cli() {
    ...
    $TOOLS_DIR/build_go_exe.sh $REPO_CLI_RELATIVE_PATH/ dcos-memcache-cli-linux linux
    $TOOLS_DIR/build_go_exe.sh $REPO_CLI_RELATIVE_PATH/ dcos-memcache-cli-darwin darwin
    $TOOLS_DIR/build_go_exe.sh $REPO_CLI_RELATIVE_PATH/ dcos-memcache-cli.exe windows
}
...
# TODO: reenable once INFINITY-2391 is resolved
...
if [ "$CLI_DIR" != $DISABLED_VALUE ]; then
    build_cli
    CLI_ARTIFACTS="${CLI_DIR}/dcos-memcache-cli-linux ${CLI_DIR}/dcos-memcache-cli-darwin ${CLI_DIR}/dcos-memcache-cli.exe"
fi
